          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {isLoading ? (
                ...
                pacientes?.filter((p) => p.tienePlanActivo).length ?? 0
              )}
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {isLoading ? (
                ...
                pacientes?.filter((p) => {
                  if (!p.fechaUltimaConsulta) return false;
                  const fecha = new Date(p.fechaUltimaConsulta);
      ...
      <div className="grid gap-4">
        {isLoading ? (
          ...
          pacientesFiltrados.map((paciente) => (
            <Card key={paciente.idPersona} className="hover:shadow-md transition-shadow">
              <CardContent className="p-4">
                <div className="flex items-center gap-4">
                  {/* Avatar */}
                  <div className="flex h-12 w-12 items-center justify-center rounded-full bg-primary/10 text-primary font-semibold">
                    {(paciente.nombre || 'U').charAt(0)}
                    {(paciente.apellido || 'N').charAt(0)}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {paciente.fechaUltimaConsulta && (
                      <p className="text-xs text-muted-foreground mt-1">
                        Última consulta:{' '}
                        {new Date(paciente.fechaUltimaConsulta).toLocaleDateString(
                          'es-AR',
                        )}
                      </p>
                    )}
                  ...
                  <div className="flex items-center gap-2">
                    <Link
                      ...
                      params={{ socioId: String(paciente.idPersona ?? '') }}
                    >
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuItem asChild>
                          ...
                          >
                            ...
                            {paciente.tienePlanActivo ? 'Editar plan' : 'Crear plan'}
                          </Link>
                        </DropdownMenuItem>
                        <DropdownMenuItem>
