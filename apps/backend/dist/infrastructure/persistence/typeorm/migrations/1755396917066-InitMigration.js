...
exports.InitMigration1755396917066 = void 0;
class InitMigration1755396917066 {
    name = 'InitMigration1755396917066';
    async up(queryRunner) {
        await queryRunner.query(`CREATE TABLE \`formacion_academica\` (\`id_formacion_academica\` int NOT NULL AUTO_INCREMENT, \`titulo\` varchar(255) NOT NULL, \`institucion\` varchar(255) NOT NULL, \`anio_inicio\` int NOT NULL, \`anio_fin\` int NOT NULL, \`nivel\` varchar(50) NOT NULL, \`id_nutricionista\` int NOT NULL, PRIMARY KEY (\`id_formacion_academica\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`ficha_salud\` (\`id_ficha_salud\` int NOT NULL AUTO_INCREMENT, \`altura\` int NOT NULL, \`peso\` decimal(5,2) NOT NULL, \`fecha_creacion\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`objetivo_personal\` varchar(255) NULL, \`nivel_actividad_fisica\` enum ('Sedentario', 'Moderado', 'Intenso') NOT NULL, PRIMARY KEY (\`id_ficha_salud\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`alimento\` (\`id_alimento\` int NOT NULL AUTO_INCREMENT, \`nombre\` varchar(255) NOT NULL, \`cantidad\` int NOT NULL, \`calorias\` int NULL, \`proteinas\` int NULL, \`carbohidratos\` int NULL, \`grasas\` int NULL, \`hidratos_de_carbono\` int NULL, \`unidad_medida\` enum ('gramo', 'kilogramo', 'mililitro', 'litro', 'miligramo', 'taza', 'cucharada', 'cucharadita') NOT NULL, PRIMARY KEY (\`id_alimento\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`opcion_comida\` (\`id_opcion_comida\` int NOT NULL AUTO_INCREMENT, \`comentarios\` varchar(255) NULL, \`tipo_comida\` enum ('DESAYUNO', 'ALMUERZO', 'MERIENDA', 'CENA', 'COLACION') NOT NULL, \`id_plan_alimentacion\` int NULL, PRIMARY KEY (\`id_opcion_comida\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`plan_alimentacion\` (\`id_plan_alimentacion\` int NOT NULL AUTO_INCREMENT, \`fechaCreacion\` date NOT NULL, \`objetivo_nutricional\` varchar(255) NOT NULL, \`id_socio\` int NOT NULL, \`id_nutricionista\` int NOT NULL, PRIMARY KEY (\`id_plan_alimentacion\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`usuario\` (\`id_usuario\` int NOT NULL AUTO_INCREMENT, \`email\` varchar(255) NOT NULL, \`contrasenia\` varchar(255) NOT NULL, \`fecha_hora_alta\` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP, \`rol\` enum ('ADMIN', 'NUTRICIONISTA', 'SOCIO') NOT NULL, \`id_persona\` int NULL, UNIQUE INDEX \`IDX_2863682842e688ca198eb25c12\` (\`email\`), UNIQUE INDEX \`REL_1fbc7de91b8e96937ed27739e8\` (\`id_persona\`), PRIMARY KEY (\`id_usuario\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`turno\` (\`id_turno\` int NOT NULL AUTO_INCREMENT, \`fecha\` date NOT NULL, \`hora_turno\` varchar(10) NOT NULL, \`estado\` enum ('PENDIENTE', 'CONFIRMADO', 'CANCELADO', 'REALIZADO', 'AUSENTE', 'REPROGRAMADO') NOT NULL, \`id_observacion\` int NULL, \`id_socio\` int NOT NULL, \`id_nutricionista\` int NOT NULL, UNIQUE INDEX \`REL_9be9fbc2d26ef165f67571ef12\` (\`id_observacion\`), PRIMARY KEY (\`id_turno\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`persona\` (\`id_persona\` int NOT NULL AUTO_INCREMENT, \`nombre\` varchar(100) NOT NULL, \`apellido\` varchar(100) NOT NULL, \`fecha_nacimiento\` date NOT NULL, \`genero\` enum ('MASCULINO', 'FEMENINO', 'OTRO') NOT NULL, \`telefono\` varchar(15) NOT NULL, \`direccion\` varchar(255) NOT NULL, \`ciudad\` varchar(100) NOT NULL, \`provincia\` varchar(100) NOT NULL, \`fecha_alta\` date NULL, \`matricula\` varchar(50) NULL, \`anios_experiencia\` int NULL, \`tarifa_sesion\` decimal(10,2) NULL, \`tipo_persona\` varchar(255) NOT NULL, \`id_ficha_salud\` int NULL, UNIQUE INDEX \`IDX_b9000711e4ac11ef438c9f405d\` (\`matricula\`), UNIQUE INDEX \`REL_db90e04e1d41b8184641cd0c43\` (\`id_ficha_salud\`), INDEX \`IDX_36e1631311d9cb31089523a5c8\` (\`tipo_persona\`), PRIMARY KEY (\`id_persona\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`agenda\` (\`id_agenda\` int NOT NULL AUTO_INCREMENT, \`dia\` enum ('Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo') NOT NULL, \`hora_inicio\` time NOT NULL, \`hora_fin\` time NOT NULL, \`duracion_turno\` int NOT NULL, \`id_nutricionista\` int NOT NULL, PRIMARY KEY (\`id_agenda\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`opcion_comida\` ADD CONSTRAINT \`FK_5ca45f7694255c195d8bc9e0c47\` FOREIGN KEY (\`id_plan_alimentacion\`) REFERENCES \`plan_alimentacion\`(\`id_plan_alimentacion\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    ...
    async down(queryRunner) {
        ...
        await queryRunner.query(`ALTER TABLE \`opcion_comida\` DROP FOREIGN KEY \`FK_5ca45f7694255c195d8bc9e0c47\``);
...
exports.InitMigration1755396917066 = InitMigration1755396917066;
//# sourceMappingURL=1755396917066-InitMigration.js.map