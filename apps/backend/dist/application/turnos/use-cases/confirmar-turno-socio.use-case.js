...
let ConfirmarTurnoSocioUseCase = class ConfirmarTurnoSocioUseCase {
    ...
    async execute(userId, turnoId) {
        ...
        if (turno.estadoTurno !== EstadoTurno_1.EstadoTurno.PENDIENTE &&
            ...
            throw new custom_exceptions_1.BadRequestError('Solo se pueden confirmar turnos en estado PENDIENTE o REPROGRAMADO.');
        }
        this.validateConfirmationWindow(turno.fechaTurno, turno.horaTurno);
        turno.estadoTurno = EstadoTurno_1.EstadoTurno.CONFIRMADO;
        const updatedTurno = await this.turnoRepository.save(turno);
        this.logger.log(`Turno ${turnoId} confirmado por socio ${socio.idPersona}.`);
