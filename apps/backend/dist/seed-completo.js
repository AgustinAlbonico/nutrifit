...
const FICHAS_SALUD = {
    'socio@nutrifit.com': {
        ...
        antecedentesFamiliares: 'Padre con diabetes tipo 2, madre con hipotiroidismo',
    ...
    'socio3@nutrifit.com': {
        ...
        cirugiasPrevias: null,
        antecedentesFamiliares: 'Padre y hermano con obesidad, madre con hipertensión',
    ...
    'socio5@nutrifit.com': {
        ...
        antecedentesFamiliares: 'Padre con cardiopatía, madre con hipercolesterolemia',
        frecuenciaComidas: '3 comidas',
        consumoAguaDiario: 2.0,
...
const AGENDA_POR_NUTRICIONISTA = {
    'nutri@nutrifit.com': [
...
async function runSeed() {
    ...
    try {
        ...
        await dataSource.query(`INSERT INTO grupo_permiso (clave, nombre, descripcion)
       ...
       WHERE NOT EXISTS (SELECT 1 FROM grupo_permiso WHERE clave = 'ADMIN')`);
        for (const accion of [...ACCIONES_PROFESIONAL, ...ACCIONES_ADMIN]) {
            await dataSource.query(`INSERT INTO accion (clave, nombre, descripcion)
         SELECT ?, ?, ?
         WHERE NOT EXISTS (SELECT 1 FROM accion WHERE clave = ?)`, [accion.clave, accion.nombre, accion.descripcion, accion.clave]);
        }
        ...
        for (const patologia of PATOLOGIAS) {
            const result = await dataSource.query(`INSERT INTO patologia (nombre) SELECT ? WHERE NOT EXISTS (SELECT 1 FROM patologia WHERE nombre = ?)`, [patologia.nombre, patologia.nombre]);
            const existing = await dataSource.query(`SELECT id_patologia FROM patologia WHERE nombre = ?`, [patologia.nombre]);
        ...
        for (const alergia of ALERGIAS) {
            const result = await dataSource.query(`INSERT INTO alergia (nombre) SELECT ? WHERE NOT EXISTS (SELECT 1 FROM alergia WHERE nombre = ?)`, [alergia.nombre, alergia.nombre]);
            const existing = await dataSource.query(`SELECT id_alergia FROM alergia WHERE nombre = ?`, [alergia.nombre]);
            const fila = obtenerPrimeraFila(existing);
            if (fila) {
                alergiaIds[alergia.nombre] = parsearNumero(fila.id_alergia) ?? 0;
            }
        }
        console.log(`   ✅ ${PATOLOGIAS.length} patologías y ${ALERGIAS.length} alergias\n`);
        ...
        const vincularUsuarioAGrupo = async (idUsuario, claveGrupo) => {
         ...
         SELECT ?, gp.id_grupo_permiso FROM grupo_permiso gp WHERE gp.clave = ?`, [idUsuario, claveGrupo]);
        };
        for (const admin of ADMINS) {
            const existente = await dataSource.query('SELECT id_usuario, id_persona FROM usuario WHERE email = ?', [admin.email]);
            const fila = obtenerPrimeraFila(existente[0]);
            let idPersona;
            let idUsuario;
            if (fila?.id_persona) {
                idPersona = parsearNumero(fila.id_persona) ?? 0;
                await dataSource.query(`UPDATE persona SET nombre = ?, apellido = ?, fecha_nacimiento = ?, genero = ?,
           ...
           WHERE id_persona = ?`, [
                ...
                const resultPersona = await dataSource.query(`INSERT INTO persona (nombre, apellido, fecha_nacimiento, genero, telefono, direccion, ciudad, provincia, dni, tipo_persona)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'AsistenteOrmEntity')`, [
                ...
                const resultUsuario = await dataSource.query(`INSERT INTO usuario (email, contrasenia, rol, id_persona) VALUES (?, ?, 'ADMIN', ?)`, [admin.email, contraseniaHash, idPersona]);
                idUsuario = obtenerInsertId(resultUsuario) ?? 0;
            }
            await dataSource.query(`UPDATE usuario SET rol = 'ADMIN' WHERE id_usuario = ?`, [idUsuario]);
        ...
        for (const nutri of NUTRICIONISTAS) {
            const existente = await dataSource.query('SELECT id_usuario, id_persona FROM usuario WHERE email = ?', [nutri.email]);
            const fila = obtenerPrimeraFila(existente[0]);
            let idPersona;
            let idUsuario;
            if (fila?.id_persona) {
           ...
           WHERE id_persona = ?`, [
                ...
                const resultPersona = await dataSource.query(`INSERT INTO persona (nombre, apellido, fecha_nacimiento, genero, telefono, direccion, ciudad, provincia, dni,
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'NutricionistaOrmEntity')`, [
                ...
                idPersona = obtenerInsertId(resultPersona) ?? 0;
                const resultUsuario = await dataSource.query(`INSERT INTO usuario (email, contrasenia, rol, id_persona) VALUES (?, ?, 'NUTRICIONISTA', ?)`, [nutri.email, contraseniaHash, idPersona]);
                idUsuario = obtenerInsertId(resultUsuario) ?? 0;
                const imagenBuffer = generarImagenAvatar(nutri.nombre, nutri.apellido, nutri.genero);
                await minioClient.putObject(bucketName, nutri.fotoPerfilKey, imagenBuffer, imagenBuffer.length, { 'Content-Type': 'image/svg+xml' });
            }
            await dataSource.query(`UPDATE usuario SET rol = 'NUTRICIONISTA' WHERE id_usuario = ?`, [idUsuario]);
        ...
        for (const socio of SOCIOS) {
            const existente = await dataSource.query('SELECT id_usuario, id_persona FROM usuario WHERE email = ?', [socio.email]);
            const fila = obtenerPrimeraFila(existente[0]);
            let idPersona;
            let idUsuario;
            if (fila?.id_persona) {
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'SocioOrmEntity')`, [
                ...
                idPersona = obtenerInsertId(resultPersona) ?? 0;
                const resultUsuario = await dataSource.query(`INSERT INTO usuario (email, contrasenia, rol, id_persona) VALUES (?, ?, 'SOCIO', ?)`, [socio.email, contraseniaHash, idPersona]);
                idUsuario = obtenerInsertId(resultUsuario) ?? 0;
                const imagenBuffer = generarImagenAvatar(socio.nombre, socio.apellido, socio.genero);
                await minioClient.putObject(bucketName, socio.fotoPerfilKey, imagenBuffer, imagenBuffer.length, { 'Content-Type': 'image/svg+xml' });
            }
            await dataSource.query(`UPDATE usuario SET rol = 'SOCIO' WHERE id_usuario = ?`, [idUsuario]);
        ...
        for (const [email, ficha] of Object.entries(FICHAS_SALUD)) {
            ...
            if (!idPersona)
                continue;
            const existente = await dataSource.query('SELECT id_ficha_salud FROM ficha_salud WHERE id_persona = (SELECT id_persona FROM persona WHERE id_persona = ?)', [idPersona]);
            let idFichaSalud;
            const fila = obtenerPrimeraFila(Array.isArray(existente[0]) ? existente[0] : existente);
            if (fila?.id_ficha_salud) {
                idFichaSalud = parsearNumero(fila.id_ficha_salud) ?? 0;
                await dataSource.query(`UPDATE ficha_salud SET altura = ?, peso = ?, objetivo_personal = ?, nivel_actividad_fisica = ?,
           ...
           WHERE id_ficha_salud = ?`, [
                ...
                await dataSource.query('DELETE FROM ficha_salud_patologias WHERE id_ficha_salud = ?', [idFichaSalud]);
                await dataSource.query('DELETE FROM ficha_salud_alergias WHERE id_ficha_salud = ?', [idFichaSalud]);
            }
            else {
                const result = await dataSource.query(`INSERT INTO ficha_salud (altura, peso, objetivo_personal, nivel_actividad_fisica,
           medicacion_actual, suplementos_actuales, cirugias_previas, antecedentes_familiares,
           frecuencia_comidas, consumo_agua_diario, restricciones_alimentarias, consumo_alcohol,
           fuma_tabaco, horas_sueno, contacto_emergencia_nombre, contacto_emergencia_telefono)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
            ...
            for (const patologiaNombre of ficha.patologias) {
                ...
                if (idPatologia) {
                    await dataSource.query('INSERT IGNORE INTO ficha_salud_patologias (id_ficha_salud, id_patologia) VALUES (?, ?)', [idFichaSalud, idPatologia]);
                }
            }
            for (const alergiaNombre of ficha.alergias) {
                const idAlergia = alergiaIds[alergiaNombre];
                if (idAlergia) {
                    await dataSource.query('INSERT IGNORE INTO ficha_salud_alergias (id_ficha_salud, id_alergia) VALUES (?, ?)', [idFichaSalud, idAlergia]);
                }
            }
        ...
        for (const [nutriEmail, agendas] of Object.entries(AGENDA_POR_NUTRICIONISTA)) {
            const idNutricionista = personaIds[nutriEmail];
            if (!idNutricionista)
                continue;
            for (const agenda of agendas) {
                await dataSource.query(`INSERT INTO agenda (dia, hora_inicio, hora_fin, duracion_turno, id_nutricionista)
           SELECT ?, ?, ?, ?, ?
           WHERE NOT EXISTS (
        ...
        for (const turno of TURNOS) {
            ...
            if (!idSocio || !idNutricionista)
                continue;
            await dataSource.query(`INSERT INTO turno (fecha, hora_turno, estado, id_socio, id_nutricionista)
         SELECT ?, ?, ?, ?, ?
         WHERE NOT EXISTS (
        ...
        for (const [socioEmail, mediciones] of Object.entries(MEDICIONES_POR_SOCIO)) {
            const idSocio = personaIds[socioEmail];
            if (!idSocio)
                continue;
            const turnosSocio = await dataSource.query(`SELECT id_turno, fecha FROM turno WHERE id_socio = ? ORDER BY fecha`, [idSocio]);
            for (let i = 0; i < mediciones.length; i++) {
                const medicion = mediciones[i];
                const turnoEncontrado = turnosSocio.find((t) => {
                    const fechaTurno = new Date(t.fecha).toISOString().split('T')[0];
                    return fechaTurno === medicion.fecha;
                });
                let idTurno;
                if (!turnoEncontrado) {
                    const nutricionistaAsignado = socioEmail === 'socio2@nutrifit.com'
                        ? personaIds['nutri2@nutrifit.com']
                        : socioEmail === 'socio4@nutrifit.com'
                            ? personaIds['nutri3@nutrifit.com']
                            : personaIds['nutri@nutrifit.com'];
                    const resultTurno = await dataSource.query(`INSERT INTO turno (fecha, hora_turno, estado, id_socio, id_nutricionista)
             VALUES (?, '10:00', 'REALIZADO', ?, ?)`, [medicion.fecha, idSocio, nutricionistaAsignado]);
                    idTurno = obtenerInsertId(resultTurno) ?? 0;
                }
                else {
                    idTurno = parsearNumero(turnoEncontrado.id_turno) ?? 0;
                }
                const existente = await dataSource.query('SELECT id_medicion FROM medicion WHERE id_turno = ?', [idTurno]);
                if (existente[0]?.length > 0)
                    continue;
                await dataSource.query(`INSERT INTO medicion (peso, altura, imc, perimetro_cintura, perimetro_cadera, perimetro_brazo,
        ...
        for (const plan of PLANES_ALIMENTACION) {
            ...
            if (!idSocio || !idNutricionista)
                continue;
            const existente = await dataSource.query('SELECT id_plan_alimentacion FROM plan_alimentacion WHERE id_socio = ? AND activo = 1', [idSocio]);
            if (existente[0]?.length > 0)
                continue;
            const resultPlan = await dataSource.query(`INSERT INTO plan_alimentacion (fechaCreacion, objetivo_nutricional, id_socio, id_nutricionista, activo)
         VALUES (CURDATE(), ?, ?, ?, 1)`, [plan.objetivoNutricional, idSocio, idNutricionista]);
            const idPlan = obtenerInsertId(resultPlan) ?? 0;
            for (const diaPlan of plan.dias) {
                const resultDia = await dataSource.query(`INSERT INTO dia_plan (dia, orden, id_plan_alimentacion) VALUES (?, ?, ?)`, [diaPlan.dia, diaPlan.orden, idPlan]);
                const idDiaPlan = obtenerInsertId(resultDia) ?? 0;
                for (const comida of diaPlan.comidas) {
                    const resultOpcion = await dataSource.query(`INSERT INTO opcion_comida (comentarios, tipo_comida, id_dia_plan) VALUES (?, ?, ?)`, [comida.comentarios, comida.tipoComida, idDiaPlan]);
        ...
        console.log(`   📋 Fichas de salud:     ${Object.keys(FICHAS_SALUD).length}`);
...
void runSeed();
//# sourceMappingURL=seed-completo.js.map