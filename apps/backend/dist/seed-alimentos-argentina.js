...
function obtenerTotalDesdeQuery(valor) {
...
const ALIMENTOS_BASE_ARGENTINA = [
    {
        nombre: 'Leche entera',
        cantidad: 100,
        calorias: 61,
...
function obtenerNutriente(nutriments, claves) {
    if (!nutriments) {
        return null;
    }
...
function resolverCantidadYUnidad(cantidadCruda) {
    if (!cantidadCruda) {
...
function mapearProductoOpenFoodFacts(producto) {
    ...
    const calorias = caloriasDirectas !== null
    ...
    return {
        ...
        carbohidratos: carbohidratosRaw !== null ? Math.round(carbohidratosRaw) : null,
        grasas: grasasRaw !== null ? Math.round(grasasRaw) : null,
        unidadMedida,
        fuente: 'OPENFOODFACTS',
    };
}
async function obtenerAlimentosOpenFoodFacts() {
    ...
    for (let pagina = 1; pagina <= MAX_PAGINAS; pagina += 1) {
        ...
        try {
            ...
            if (!response.ok) {
                console.warn(`⚠ Open Food Facts devolvió ${response.status} en página ${pagina}.`);
                continue;
            }
...
async function insertarLote(dataSource, lote) {
    if (lote.length === 0) {
        return;
    }
    ...
    for (const alimento of lote) {
        params.push(alimento.nombre, alimento.cantidad, alimento.calorias, alimento.proteinas, alimento.carbohidratos, alimento.grasas, alimento.carbohidratos, alimento.unidadMedida);
    }
    await dataSource.query(`INSERT INTO alimento
...
async function runSeedAlimentosArgentina() {
    ...
    if (!host || !Number.isFinite(port) || !username || !database) {
        throw new Error('Faltan variables de base de datos (DATABASE_HOST, DATABASE_PORT, DATABASE_USER, DATABASE_NAME).');
    }
    ...
    try {
        ...
        console.log(`📦 Open Food Facts: ${alimentosOpenFoodFacts.length} alimentos candidatos.`);
        ...
        const filasExistentesRaw = (await dataSource.query('SELECT nombre FROM alimento'));
        const filasExistentes = Array.isArray(filasExistentesRaw)
            ? filasExistentesRaw.filter(esFilaNombre)
            : [];
        const nombresExistentes = new Set(filasExistentes
            .map((fila) => normalizarNombre(fila.nombre))
            .filter((nombre) => nombre.length > 0));
        const nuevos = candidatos.filter((alimento) => !nombresExistentes.has(normalizarNombre(alimento.nombre)));
        ...
        const totalRaw = (await dataSource.query('SELECT COUNT(*) AS total FROM alimento'));
...
//# sourceMappingURL=seed-alimentos-argentina.js.map